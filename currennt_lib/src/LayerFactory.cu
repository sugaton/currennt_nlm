        ...
        int maxSeqLength, layers::Layer<TDevice> *precedingLayer)
{
    ...
    if (layerType == "input")
    	...
    	return new intInputLayer<TDevice>(layerChild, parallelSequences, maxSeqLength);
    else if (layerType == "lookup")
    	return new LookupLayer<TDevice>(layerChild, weightsSection, *precedingLayer);
